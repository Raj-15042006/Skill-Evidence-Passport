...
import { evaluateEvidenceWithAI } from '../src/services/aiEngine.js';
import { AuditLedger, computeLogHash } from '../src/services/auditEngine.js';
import { decryptData, encryptData, generateHMACSignature, verifyHMACSignature } from '../server/crypto.js';
import { verifySubmittedDocument } from '../src/services/documentVerifier.js';
...
runTest('Audit Engine - Tamper Detection', () => {
  const ledger = new AuditLedger();
  ledger.appendLog('usr-1', 'Alex', 'student', 'SUBMIT', 'EVIDENCE', 'ev-1', 'Initial payload');
  ledger.appendLog('usr-2', 'Sarah', 'verifier', 'APPROVE', 'EVIDENCE', 'ev-1', 'Approved payload');
...
runTest('Document Verifier - Valid File Format', () => {
  const res = verifySubmittedDocument(
    ...
    'DOCUMENT',
    'High Performance React Virtualization',
    'Detailed implementation of virtualized rendering engine handling 100k data points.',
    []
  );
...
runTest('Document Verifier - Duplicate SHA-256 Hash Detection', () => {
  const mockExisting = [
    {
      ...
      description: 'Duplicate description text',
      fileRef: 'repo.zip',
      status: 'APPROVED',
      submittedAt: '',
      updatedAt: '',
      contentHash: Array.from('Duplicate TitleDuplicate description textrepo.zip')
  ...
  const res = verifySubmittedDocument('repo.zip', '', 'REPOSITORY', 'Duplicate Title', 'Duplicate description text', mockExisting);
  assert.strictEqual(res.isVerified, false);
  assert.ok(res.errors.some((e) => e.includes('Duplicate submission detected')));
});

// 4. AI Engine Advisory Pre-Screening Tests
runTest('AI Engine - Advisory Pre-Screening & Cosine Similarity', () => {
  const mockSkill = {
    id: 'sk-react',
